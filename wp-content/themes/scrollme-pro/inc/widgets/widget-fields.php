...
function scrollme_widgets_show_widget_field($instance = '', $widget_field = '', $athm_field_value = '') {
    // Store Posts in array
    $scrollme_postlist[0] = array(
        'value' => 0,
        'label' => '--choose--'
    );
    $arg = array('posts_per_page' => -1);
    $scrollme_posts = get_posts($arg);
    foreach ($scrollme_posts as $scrollme_post) :
        $scrollme_postlist[$scrollme_post->ID] = array(
            'value' => $scrollme_post->ID,
            'label' => $scrollme_post->post_title
        );
    endforeach;

    extract($widget_field);

    switch ($scrollme_widgets_field_type) {

        // Standard text field
        case 'text' :
            ?>
            <p>
                <label for="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>"><?php echo $scrollme_widgets_title; ?>:</label>
                <input class="widefat" id="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>" name="<?php echo $instance->get_field_name($scrollme_widgets_name); ?>" type="text" value="<?php echo $athm_field_value; ?>" />

                <?php if (isset($scrollme_widgets_description)) { ?>
                    <br />
                    <small><?php echo $scrollme_widgets_description; ?></small>
                <?php } ?>
            </p>
            <?php
            break;
        ...
        case 'textarea' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>"><?php echo $scrollme_widgets_title; ?>:</label>
                <textarea class="widefat" rows="<?php echo $scrollme_widgets_row; ?>" id="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>" name="<?php echo $instance->get_field_name($scrollme_widgets_name); ?>"><?php echo $athm_field_value; ?></textarea>
            </p>
            <?php if (isset($scrollme_widgets_description)) { ?>
            <small><?php echo $scrollme_widgets_description; ?></small>
        <?php } ?>
        ...
        case 'checkbox' :
            ...
            <p>
                ...
                <input id="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>" name="<?php echo $instance->get_field_name($scrollme_widgets_name); ?>" type="checkbox" value="1" <?php checked('1', $athm_field_value); ?>/>
                <label for="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>"><?php  echo $scrollme_widgets_title; ?></label>
        ...
        case 'radio' :
            ...
            <p>
                <?php
                echo $scrollme_widgets_title;
                echo '<br />';
                foreach ($scrollme_widgets_field_options as $athm_option_name => $athm_option_title) {
                    ?>
                    <input id="<?php echo $instance->get_field_id($athm_option_name); ?>" name="<?php echo $instance->get_field_name($scrollme_widgets_name); ?>" type="radio" value="<?php echo $athm_option_name; ?>" <?php checked($athm_option_name, $athm_field_value); ?> />
        ...
        case 'select' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>"><?php echo $scrollme_widgets_title; ?>:</label>
                <select name="<?php echo $instance->get_field_name($scrollme_widgets_name); ?>" id="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>" class="widefat">
                    <?php foreach ($scrollme_widgets_field_options as $athm_option_name => $athm_option_title) { ?>
        ...
        case 'number' :
            ?>
            <p>
                <label for="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>"><?php echo $scrollme_widgets_title; ?>:</label><br />
                <input name="<?php echo $instance->get_field_name($scrollme_widgets_name); ?>" type="number" step="1" min="1" id="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>" value="<?php echo $athm_field_value; ?>" class="small-text" />
        ...
        case 'selectpost' :
            ...
            <p>
                <label for="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>"><?php echo $scrollme_widgets_title; ?>:</label>
                <select name="<?php echo $instance->get_field_name($scrollme_widgets_name); ?>" id="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>" class="widefat">
                    <?php foreach ($scrollme_postlist as $scrollme_single_post) { ?>
                        <option value="<?php echo $scrollme_single_post['value']; ?>" id="<?php echo $instance->get_field_id($scrollme_single_post['label']); ?>" <?php selected($scrollme_single_post['value'], $athm_field_value); ?>><?php echo $scrollme_single_post['label']; ?></option>
                    <?php } ?>
                </select>
        ...
        case 'upload' :
            ...
            $output = '';
            $id = $instance->get_field_id($scrollme_widgets_name);
            $class = '';
            $int = '';
            $value = $athm_field_value;
            $name = $instance->get_field_name($scrollme_widgets_name);
            ...
            $output .= '<label for="' . $instance->get_field_id($scrollme_widgets_name) . '">' . $scrollme_widgets_title . '</label><br/>';
            $output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="' . $name . '" value="' . $value . '" placeholder="' . __('No file chosen', 'scrollme-pro') . '" />' . "\n";
            if (function_exists('wp_enqueue_media')) {
                if (( $value == '')) {
                    $output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __('Upload', 'scrollme-pro') . '" />' . "\n";
                } else {
                    $output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __('Remove', 'scrollme-pro') . '" />' . "\n";
                }
            } else {
                $output .= '<p><i>' . __('Upgrade your version of WordPress for full media support.', 'scrollme-pro') . '</i></p>';
            }
            ...
            if ($value != '') {
                ...
                if ($image) {
                    ...
                    $title = __('View File', 'scrollme-pro');
        ...
        case 'icon' :
            ...
            <p>
                ...
                <input class="hidden-icon-input" name="<?php echo $instance->get_field_name($scrollme_widgets_name); ?>" type="hidden" id="<?php echo $instance->get_field_id($scrollme_widgets_name); ?>" value="<?php echo $athm_field_value; ?>" />
...
function scrollme_widgets_updated_field_value($widget_field, $new_field_value) {

    extract($widget_field);

    // Allow only integers in number fields
    if ($scrollme_widgets_field_type == 'number') {
        return absint($new_field_value);

        // Allow some tags in textareas
    } elseif ($scrollme_widgets_field_type == 'textarea') {
        // Check if field array specifed allowed tags
        if (!isset($scrollme_widgets_allowed_tags)) {
            // If not, fallback to default tags
            $scrollme_widgets_allowed_tags = '<p><strong><em><a><li>';
        }
        return strip_tags($new_field_value, $scrollme_widgets_allowed_tags);

        // No allowed tags for all other fields
    } elseif($scrollme_widgets_field_type == 'textfield'){
        if (!isset($scrollme_widgets_allowed_tags)) {
            // If not, fallback to default tags
            $scrollme_widgets_allowed_tags = '<span>';
        }
        return strip_tags($new_field_value, $scrollme_widgets_allowed_tags);
    } elseif ($scrollme_widgets_field_type == 'url') {
